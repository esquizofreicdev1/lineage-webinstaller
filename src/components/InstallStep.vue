<template>
    <v-container class="d-flex justify-space-between flex-column flex-grow-1">
        <div class="mt-n4 flex-grow-1" v-if="$root.$data.release !== null">
            <h6 class="text-h6 pb-4">Instalar LineageOS 18.1</h6>

            <div class="text-body-1">
                <p>
                    Esto instalara LineageOS 18.1
                    {{ getDeviceName($root.$data.product) }}.
                </p>
                <p v-if="$root.$data.installType === 'clean'">
                    Instalacion limpia, esto borrara sus datos
                    <strong class="red--text text--darken-3"
                        >Esto borrara todos sus datos!</strong
                    >
                </p>
                <p>
                    <strong>No toque el dispositivo mientras se instala LineageOS 18.1</strong>
                </p>
                <p>
                    Su dispositivo se reiniciara varias veces no lo toque
                    <strong>NO LO TOQUE POR FAVOR!</strong> Mire la barra de progreso
                </p>
            </div>
    ...
    methods: {
        ...
        async install() {
            ...
            try {
                ...
                let product = await this.device.getVariable("product");
                if ( product == "tucana")
                {
                ...
                }else if (product == "apollo")
                {
                    await this.device.flashFactoryZipApollo(
                    blob,
                    this.$root.$data.installType === "clean",
                    this.reconnectCallback,
                    (action, item, progress) => {
                ...
                this.installStatus = `Reiniciando en LineageOS 18.1!`;
